# simulator-session-isolation Specification

## Purpose

使 `user_simulator` 的对话记忆边界与 agent 的 gateway session 边界一一对齐:同一 `session_name` 下的 query 复用同一份对话记忆,不同 `session_name` 之间相互不可见,从而杜绝跨会话信息泄露;并在记忆为空且 query 含无解指代时,让模拟用户据实应对而非靠泄露蒙混推进任务。
