...
class HarnessAutomation:
    ...
    def __init__(self, config: AutomationConfig):
        ...
        if self.harness_type == "hermes":
            ...
            from src.grok_client import (
                ...
                resolve_grok_workspace_root,
            )
            self.workspace_manager = GrokWorkspaceManager(
                str(resolve_grok_workspace_root())
            )
        else:
    ...
    async def _run_grok(self) -> Dict[str, Any]:
        from src.grok_client import (
            GrokAgentManager,
            build_grok_client,
        ...
        async with await build_grok_client() as client:
            ...
            # 先构造 manager 完成 $GROK_HOME/workspace 重叠校验，再写入模板。
            await self._setup_workspaces()
